...
func Execute(functionName string, properties common.FunctionProperties, event []byte) *LocalFunctionResponseMessage {
	...
	header[proxy.HeaderContentType] = proxy.TypeJson
	header[proxy.HeaderRequestId] = requestId
	header[proxy.HeaderFunctionId] = functionId
	header[proxy.HeaderFunctionName] = functionName
	header[proxy.HeaderFunctionVersion] = "csa-local-v1.0"
	header[proxy.HeaderFunctionHandler] = properties.Handler
	header[proxy.HeaderFunctionMemory] = strconv.Itoa(properties.MemorySize)
	header[proxy.HeaderFunctionTimeout] = strconv.Itoa(properties.Timeout)
	header[proxy.HeaderFunctionMD5] = fmt.Sprintf("%s-%d", "csa-md5", time.Now().Unix())
	header[proxy.HeaderFunctionCodePath] = TARGET_VOL_PATH
	env, _ := json.Marshal(properties.Env)
	header[proxy.HeaderFunctionEnv] = string(env)

	postData, err := json.Marshal(event)
	if err != nil {
		res.Code = ferror.InternalErrorCode
		return res
	}
	wsgiClient := http_helper.NewHttpClient()
	httpRsp, err := wsgiClient.Forward(WsgiUrl, http.MethodPost, bytes.NewReader(postData), header)
	if err != nil {
		res.Stderr = err.Error()
		res.Code = ferror.InvokeFunctionAskCode
		return res
	}
	...
	wsgiRes := new(proxy.WsgiResponse)
	...
	if httpRsp.StatusCode != 200 {
		if httpRsp.StatusCode == 400 {
			res.Code = ferror.UserFuncExecuteError
		} else {
			res.Code = ferror.InvokeFunctionAskCode
		}
		return res
	...
	res.Code = ferror.InternalSuccessCode
	return res
}
